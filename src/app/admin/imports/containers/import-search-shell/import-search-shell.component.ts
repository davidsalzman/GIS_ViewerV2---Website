...
@Component({
  selector: 'app-import-search-shell',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './import-search-shell.component.html',
  styleUrls: ['./import-search-shell.component.scss']
})
export class ImportSearchShellComponent implements OnInit, OnDestroy {
  faSearch = faSearch;
  faBook = faBook;
  faPlus = faPlus;
  faTimes = faTimes;
  ...
  public pageNumber = 1;
  public result: ISearchDTO<IImportDTO> = {
    total: 0,
    items: []
  };
  ...
  public onShowDelete(id: any): void {
    ...
    this.subscriptions.push(onClose.subscribe(async (doDelete: boolean) => {
      if (doDelete && id) {
        try {
          await this.api.imports.deleteAsync(id)
  ...
  public async onSearch() {
    try {
      ...
      this.result = await this.api.imports.searchAsync(contract);
  ...
  public async onToggleImport(importId: any, event: any): Promise<void> {
    if (!this.isSubmitting) {
      this.isSubmitting = true;

      try {

        await this.api.imports.toggleImportUpdateAsynch(importId, {
          isActive: event.target.checked

        });
        this.updateQueryParams();
      } catch (error) {
        this.messageService.exception(error, 'Failed to update import, try again!');
      } finally {
